...
def combinar_gtas(gtas_oe, gtas_mg):
    gtas_oe = gtas_oe.rename(columns={
        'municipio_origem': 'nome_municipio_origem',
        'municipio_destino': 'nome_municipio_destino'
    })
...
def combinar_distancias_gtas(dist_csv, df_gta):
    ...
    df_dist = df_dist.drop_duplicates(subset=['municipio_origem', 'municipio_destino'])
    df_dist = df_dist.rename(columns={
        'municipio_origem': 'nome_municipio_origem',
        'municipio_destino': 'nome_municipio_destino'
    })
    df_gta['nome_municipio_origem'] = df_gta['nome_municipio_origem'].str.upper()
    df_gta['nome_municipio_destino'] = df_gta['nome_municipio_destino'].str.upper()
    df = df_gta.merge(df_dist, on=['nome_municipio_origem', 'nome_municipio_destino'], how='left', validate='many_to_one')
    df['distancia_km'] = df.apply(
        lambda r: r['dist_ped_km'] if r['ds_meio_transporte'] == 'A PÉ' else r['dist_rod_km'],
        axis=1
    )
    df['duracao_min'] = df.apply(
        lambda r: r['dist_ped_min'] if r['ds_meio_transporte'] == 'A PÉ' else r['dist_rod_min'],
        axis=1
    )
